...
export function PublicProfilePage() {
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '56px 20px 80px' }}>
        ...
        <div className="anim-fade-up" style={{ textAlign: 'center', marginBottom: 36 }}>
          ...
          {user.bio && (
            <p style={{
              ...
              maxWidth: 320, margin: '10px auto 0', lineHeight: 1.6,
            }}>
              {user.bio}
            </p>
          )}
        ...
        {/* Stats */}
        {stats.totalTasks > 0 && (
          <div className="anim-fade-up delay-100" style={{ background: '#fff', borderRadius: 20, border: '1px solid rgba(124,58,237,0.1)', padding: '20px 24px', marginBottom: 20, boxShadow: '0 4px 20px rgba(124,58,237,0.06)' }}>
              ...
              <div style={{ width: 90, height: 90, flexShrink: 0 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <RadialBarChart innerRadius="65%" outerRadius="100%" data={[{ value: stats.onTimePercent, fill: '#7c3aed' }]} startAngle={90} endAngle={-270}>
                    ...
                    <RadialBar dataKey="value" cornerRadius={6} background={{ fill: '#f3e8ff' }} />
                  </RadialBarChart>
                </ResponsiveContainer>
              </div>
              <div style={{ flex: 1 }}>
        ...
        {boardsData && boardsData.length > 0 && (
          <div className="anim-fade-up delay-300" style={{ marginTop: 28 }}>
...
function LinkButton({ link, delay }: { link: PublicLink; delay: number }) {
  ...
  const isDark = ['#000000', '#010101', '#24292e', '#1b1b26'].includes(bg);
  ...
  return (
    <a
      ...
      style={{
        ...
        boxShadow: hov
          ? `0 8px 24px ${bg === '#000000' ? 'rgba(0,0,0,0.35)' : `${bg}60`}`
          : `0 3px 12px ${bg === '#000000' ? 'rgba(0,0,0,0.2)' : `${bg}35`}`,
...
function PublicBoardCard({ board, delay }: { board: PublicBoard; delay: number }) {
  ...
  return (
    <div
      className="anim-fade-up"
      style={{ animationDelay: `${delay}ms`, background: '#fff', borderRadius: 16, border: '1px solid rgba(124,58,237,0.1)', overflow: 'hidden', boxShadow: hov ? '0 8px 28px rgba(124,58,237,0.12)' : '0 2px 10px rgba(124,58,237,0.06)', transition: 'all 0.22s var(--ease-spring)', transform: hov ? 'translateY(-2px)' : 'translateY(0)' }}
      onMouseEnter={() => setHov(true)}
      onMouseLeave={() => setHov(false)}>
      {/* Color bar */}
              ...
              return (
                <div key={list.id} style={{ marginTop: 14 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {list.cards.length === 0
                      ? <div style={{ fontSize: 12, color: 'var(--fig-text-muted)', padding: '6px 10px', borderRadius: 8, background: '#f8f7ff', fontStyle: 'italic' }}>No cards</div>
                      : list.cards.map((card) => (
...
function PublicCardItem({ card, listColor }: { card: PublicCard; listColor: string }) {
  ...
  return (
    <div
      style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '8px 12px', borderRadius: 10, background: hov ? '#f5f3ff' : '#fafafa', border: `1px solid ${hov ? 'rgba(124,58,237,0.18)' : 'rgba(0,0,0,0.06)'}`, transition: 'all 0.18s', cursor: 'default', position: 'relative', overflow: 'hidden' }}
      onMouseEnter={() => setHov(true)}
      onMouseLeave={() => setHov(false)}>
...
function ProfileNotFound({ username }: { username: string }) {
  return (
    <div style={{
      ...
      background: 'linear-gradient(160deg, #f8f7ff, #eff6ff)',
